...
console.log('\n[20] Embedded finance');
const payroll = await req('GET', '/api/v1/finance/payroll-summary?days=14', { token: admin });
check('payroll summary (4 staff + totals)', payroll.status === 200 && payroll.data?.staff?.length === 4 && payroll.data?.totals?.wages >= 0, JSON.stringify(payroll.data).slice(0, 150));
check('tips flow from time_logs (15 declared in [11])', (payroll.data?.totals?.tips ?? 0) >= 15, 'got ' + payroll.data?.totals?.tips);
const adv1 = await req('POST', '/api/v1/finance/advances', { token: admin, body: { staffId, amount: 120, reason: 'car repair' } });
check('request advance → pending', adv1.status === 201 && adv1.data.status === 'pending', JSON.stringify(adv1.data).slice(0, 120));
const advApprove = await req('POST', `/api/v1/finance/advances/${adv1.data.id}/approve`, { token: admin, body: {} });
check('approve advance → approved_at stamped', advApprove.status === 200 && advApprove.data.status === 'approved' && !!advApprove.data.approved_at, JSON.stringify(advApprove.data).slice(0, 120));
const advRepaid = await req('POST', `/api/v1/finance/advances/${adv1.data.id}/repay`, { token: admin, body: {} });
check('repay advance → repaid_at stamped', advRepaid.status === 200 && advRepaid.data.status === 'repaid' && !!advRepaid.data.repaid_at, JSON.stringify(advRepaid.data).slice(0, 120));
const exp1 = await req('POST', '/api/v1/finance/expenses', { token: admin, body: { category: 'supplies', vendor: 'Smoke Suppl.', amount: 85.5, notes: 'cleaning' } });
check('record expense', exp1.status === 201 && exp1.data.category === 'supplies', JSON.stringify(exp1.data).slice(0, 120));
const expSum = await req('GET', '/api/v1/finance/expense-summary', { token: admin });
check('expense summary: seeded utilities + new supplies', expSum.status === 200 && expSum.data.by_category.some((c) => c.category === 'utilities') && expSum.data.by_category.some((c) => c.category === 'supplies'), JSON.stringify(expSum.data));
const expDel = await req('DELETE', `/api/v1/finance/expenses/${exp1.data.id}`, { token: admin });
check('delete expense', expDel.status === 200, JSON.stringify(expDel.data));

console.log('\n[21] Training system');
const tCourses = await req('GET', '/api/v1/training/courses', { token: admin });
check('courses seeded (3, of which 2 required)', tCourses.status === 200 && tCourses.data.length === 3 && tCourses.data.filter((c) => c.required).length === 2, JSON.stringify(tCourses.data?.map((c) => c.title)));
const tOv = await req('GET', '/api/v1/training/overview', { token: admin });
check('team overview (4 staff)', tOv.status === 200 && tOv.data.staff.length === 4, 'got ' + tOv.data?.staff?.length);
check('compliance 0% before completions (8 required slots)', tOv.data.compliance.required_slots === 8 && tOv.data.compliance.compliance_pct === 0, JSON.stringify(tOv.data.compliance));
const tEnroll = await req('POST', '/api/v1/training/enroll', { token: admin, body: { staffId, courseId: tCourses.data[2].id } });
check('enroll in optional course (idempotent)', tEnroll.status === 201, JSON.stringify(tEnroll.data).slice(0, 100));
const tProg = await req('POST', '/api/v1/training/progress', { token: admin, body: { staffId, courseId: tCourses.data[0].id, progress: 100, quizResult: [0, 1] } });
check('quiz 2/2 → completed with score 100', tProg.status === 200 && tProg.data.score === 100 && !!tProg.data.completed_at, JSON.stringify(tProg.data).slice(0, 150));
const tProg2 = await req('POST', '/api/v1/training/progress', { token: admin, body: { staffId, courseId: tCourses.data[1].id, progress: 100, quizResult: [1] } });
check('quiz 0/1 → NOT completed despite 100% progress', tProg2.status === 200 && tProg2.data.completed_at === null && tProg2.data.score === 0, JSON.stringify(tProg2.data).slice(0, 120));
const tOv2 = await req('GET', '/api/v1/training/overview', { token: admin });
check('compliance improved (1/8 = 12.5% → 13)', tOv2.data.compliance.compliance_pct === 13, 'got ' + tOv2.data.compliance.compliance_pct + '%');
const tNew = await req('POST', '/api/v1/training/courses', { token: admin, body: { title: 'Smoke Course', category: 'custom', duration_min: 10, required: false, quiz: [] } });
check('create course → 201', tNew.status === 201, JSON.stringify(tNew.data).slice(0, 100));
const tDel = await req('DELETE', `/api/v1/training/courses/${tNew.data.id}`, { token: admin });
check('delete course', tDel.status === 200, JSON.stringify(tDel.data));

console.log('\n[22] Vendor marketplace');
const vendors = await req('GET', '/api/v1/vendors', { token: admin });
check('suppliers seeded (3)', vendors.status === 200 && vendors.data.length === 3, 'got ' + vendors.data?.length);
const compare = await req('GET', '/api/v1/vendors/compare', { token: admin });
check('cheapest-source comparison (7 linked ingredients)', compare.status === 200 && compare.data.length === 7, 'got ' + compare.data?.length);
const lettuceRow = compare.data.find((c) => c.name === 'Lettuce');
check('lettuce best source = Metro Produce @ 0.90', lettuceRow && lettuceRow.best_supplier === 'Metro Produce Co.' && Math.abs(lettuceRow.best_price - 0.9) < 0.001, JSON.stringify(lettuceRow));
const meatRow = compare.data.find((c) => c.name === 'Beef Patty');
check('beef patty best source = Summit Meats @ 2.40', meatRow && meatRow.best_supplier === 'Summit Meats & Dairy' && Math.abs(meatRow.best_price - 2.4) < 0.001, JSON.stringify(meatRow));
const metro = vendors.data.find((v) => v.name === 'Metro Produce Co.');
const supProducts = await req('GET', `/api/v1/vendors/${metro.id}/products`, { token: admin });
check('supplier products listed (2)', supProducts.status === 200 && supProducts.data.length === 2, 'got ' + supProducts.data?.length);
const lettuceProd = supProducts.data.find((p) => p.name === 'Lettuce');
const lettuceBefore = (await db.query(`SELECT current_stock FROM ingredients WHERE name = 'Lettuce'`)).rows[0];
const vendOrder = await req('POST', '/api/v1/vendors/orders', { token: admin, body: { supplierId: metro.id, items: [{ productId: lettuceProd.id, qty: 12 }] } });
check('place purchase order → sent, total 10.80', vendOrder.status === 201 && vendOrder.data.status === 'sent' && Math.abs(Number(vendOrder.data.total) - 10.8) < 0.001, JSON.stringify(vendOrder.data).slice(0, 150));
const vendMin = await req('POST', '/api/v1/vendors/orders', { token: admin, body: { supplierId: metro.id, items: [{ productId: lettuceProd.id, qty: 5 }] } });
check('below min-order rejected → 400', vendMin.status === 400, 'got ' + vendMin.status);
const vendReceive = await req('POST', `/api/v1/vendors/orders/${vendOrder.data.id}/receive`, { token: admin, body: {} });
check('receive order → lettuce matched for stocking', vendReceive.status === 200 && vendReceive.data.stocked?.some((s) => s.matched), JSON.stringify(vendReceive.data.stocked));
const lettuceAfter = (await db.query(`SELECT current_stock FROM ingredients WHERE name = 'Lettuce'`)).rows[0];
check('lettuce stock +12 on receipt', Number(lettuceAfter.current_stock) === Number(lettuceBefore.current_stock) + 12, `${lettuceBefore.current_stock} → ${lettuceAfter.current_stock}`);
const purchTx = await db.query(`SELECT count(*)::int n FROM stock_transactions WHERE reason = 'purchase'`);
check('purchase audit row written', purchTx.rows[0].n === 1, 'got ' + purchTx.rows[0].n);
